============================================================
  CLAUDE CODE — TAM SİSTEM REHBERİ
  Güncelleme: 2026-04-19 (v2 — düzeltmeler uygulandı)
============================================================
...
Bu sistem 2026-04-19 kuruldu, aynı gün kritik hatalar düzeltildi.

  • 1 global CLAUDE.md (her oturumda GERÇEKTEN otomatik aktif)
  ...
  • GitHub backup: github.com/BuraakAI/claude-setup


─────────────────────────────────────────────────────────
  A) GLOBAL CLAUDE.MD — OTOMATIK AKTİF KURALLAR
─────────────────────────────────────────────────────────

  Dosya: ~/.claude/CLAUDE.md
  Bu dosya her oturumda Claude tarafından otomatik okunur.
  Hiçbir şey yazmana gerek yok — her zaman aktif.
  ...
  NASIL ÇALIŞIR (LOG SİSTEMİ):
    1. Oturum başında: Claude PROGRESS.md'yi okur (varsa)
    2. İş yapılır
    3. Oturum sonunda:
       - Claude, PROGRESS.md'ye ne yapıldığını yazar (CLAUDE.md zorlar)
       - Stop hook (log-session.ps1), timestamp ekler:
         "--- Oturum kapandı: 2026-04-19 16:45 ---"
    Sonuç: PROGRESS.md hem içerik hem zaman damgası alır.


─────────────────────────────────────────────────────────
  B) GLOBAL SKİLL'LER  (~/.claude/skills/)
─────────────────────────────────────────────────────────

  Skills @ ile ELLE çağrılır — otomatik aktif DEĞİL.
  (Otomatik aktif olan = ~/.claude/CLAUDE.md — bkz. A bölümü)

┌─────────────────────┬──────────────────────────────────────────────────┐
│ Skill               │ Açıklama & Kullanım                              │
├─────────────────────┼──────────────────────────────────────────────────┤
│ @default-mode       │ Global CLAUDE.md'yi tamamlar. Ek Flutter         │
│                     │ kuralları, repository pattern, tip güvenliği.    │
│                     │ Detaylı oturum için çağır.                       │
├─────────────────────┼──────────────────────────────────────────────────┤
│ @token-saver        │ Kısa mod. Token israfını keser.                  │
│                     │ Kullanım: @token-saver                           │
├─────────────────────┼──────────────────────────────────────────────────┤
│ @build-feature      │ Yeni özellik için tam scaffold üretir.           │
│                     │ Kullanım: @build-feature auth "email ile giriş"  │
│                     │ Çıktı: feature klasörü + repo + provider + route │
├─────────────────────┼──────────────────────────────────────────────────┤
│ @bug-hunter         │ Root cause analizi. Stack trace yapıştır.        │
│                     │ Çıktı: HATA / NEDEN / FİX formatında             │
├─────────────────────┼──────────────────────────────────────────────────┤
│ @supabase-expert    │ Schema tasarımı, RLS policy, migration akışı,    │
│                     │ Flutter entegrasyon pattern'leri.                │
├─────────────────────┼──────────────────────────────────────────────────┤
│ @new-project        │ Sıfırdan Flutter+Supabase proje iskeleti.        │
│                     │ Kullanım: @new-project uygulama-adı "açıklama"   │
├─────────────────────┼──────────────────────────────────────────────────┤
│ @seo-optimizer      │ Web SEO, Core Web Vitals, keyword, schema markup │
├─────────────────────┼──────────────────────────────────────────────────┤
│ @excalidraw-skill   │ Diyagram ve roadmap çizimi (MCP gerekli).        │
├─────────────────────┼──────────────────────────────────────────────────┤
│ @security-audit     │ Flutter+Supabase+IAP+RevenueCat güvenlik taraması│
│                     │ Hardcoded key, RLS boşluğu, webhook auth,        │
│                     │ IAP server-side doğrulama, obfuscation kontrol.  │
│                     │ Çıktı: KRİTİK / ORTA / DÜŞÜK / TEMİZ raporu     │
└─────────────────────┴──────────────────────────────────────────────────┘
...
── 1. EXCALIDRAW MCP (Global — tüm projelerde) ──────────
  ...
  26 araç: element CRUD, hizalama, gruplama, screenshot,
           snapshot, Mermaid→Excalidraw, export/import
...
─────────────────────────────────────────────────────────
  D) OTOMATİK HOOK  (~/.claude/settings.json)
─────────────────────────────────────────────────────────

  !! SMART-COMMIT KALDIRILDI — yarım commit, bozuk git geçmişi riski
  !! Git commit'leri manuel yapılır veya proje bazlı eklenir
...
── STOP HOOK — Oturum Log Timestamp'i ──────────────────
  ...
  PROGRESS.md içeriğini Claude yazar (CLAUDE.md protokolü),
  timestamp'i stop hook ekler. İkisi birlikte tam log oluşturur.
...
─────────────────────────────────────────────────────────
  ...
  ~/.claude/
    ...
    templates/
      CLAUDE.md            → Yeni proje şablonu
      .gitignore           → Flutter/Supabase güvenli
      mcp-supabase-example.json
  ...
  Tools:
    ...
    C:\Users\burak\tools\claude-setup\     → GitHub backup repo


─────────────────────────────────────────────────────────
  J) GITHUB BACKUP & EKİP KURULUMU
─────────────────────────────────────────────────────────

  Repo   : github.com/BuraakAI/claude-setup (public)
  Güncelle: claude-setup klasöründe değişiklik yap → commit → push

  Ekip üyesi kurulumu (tek komut):
    Windows: irm https://raw.githubusercontent.com/BuraakAI/claude-setup/main/install.ps1 | iex
    Mac/Linux: curl -fsSL https://raw.githubusercontent.com/BuraakAI/claude-setup/main/install.sh | bash

  Sistem güncellemesi geldiğinde:
    Aynı komutu tekrar çalıştır — mevcut ayarları silmez, merge eder.
